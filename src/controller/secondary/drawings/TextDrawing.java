...
public class TextDrawing implements Drawing{
	...
	/*	if(counter == 2)
		{
			if(AppConfig.drawings.contains(AppConfig.selected))
			{
				counter++;
			...
			{
				AppConfig.drawingArea.setSize(AppConfig.size.width, AppConfig.size.height);
				counter++;
				return transformOnCenter(AffineTransform.getRotateInstance(theta));
			}
		}
		...
		else
		{
			...
			{
				AppConfig.drawingArea.setSize(AppConfig.size.height, AppConfig.size.width);
				counter++;
				return transformOnCenter(AffineTransform.getRotateInstance(theta));
			}
		}*/
	...
	public AffineTransform quadrantRotate(int numquadrants, double anchorx,
			double anchory) {
		return updateTransform(bound.quadrantRotate(numquadrants, anchorx,
				anchory));
	}
